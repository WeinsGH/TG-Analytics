...
async def parsing(chanels_user_input, uid):
    ...
    with open(f'ldb/{uid}.csv', "w", encoding='UTF-8', newline='') as f:
        writer = csv.writer(f, delimiter=",", lineterminator="\n")

        # Если файл только что создан, добавляем заголовок
        if not os.path.exists(f'ldb/{uid}.csv') or os.path.getsize(f'ldb/{uid}.csv') == 0:
